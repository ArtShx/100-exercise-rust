...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleErr{ msg: "Title cannot be empty".to_string() });
        }
        if title.len() > 50 {
            return Err(TicketNewError::TitleErr{ msg: "Title cannot be longer than 50 characters".to_string() });
        }
        if description.is_empty() {
            return Err(TicketNewError::DescriptionErr{ msg: "Title cannot be longer than 50 characters".to_string() });
        }
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionErr{ msg: "Description cannot be longer than 500 characters".to_string() });
        }
