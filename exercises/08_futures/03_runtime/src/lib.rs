...
use std::{fmt::{Display, Pointer}, panic};
    ...
    T: Display + Send + Sync + 'static,
{
    ...
    let ref_reply = Arc::new(reply);
    let handle1 = tokio::spawn(echo(first, ref_reply.clone()));
    let handle2 = tokio::spawn(echo(second, ref_reply.clone()));
