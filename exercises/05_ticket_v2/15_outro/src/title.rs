...
fn validate(value: &str) -> Result<(), TicketParsingErr> {
    if value.is_empty() {
        return Err(TicketParsingErr::Empty);
    }
    if value.len() > 50 {
        return Err(TicketParsingErr::TooLong);
    }
    Ok(())
}

#[derive(thiserror::Error, Debug)]
pub enum TicketParsingErr{    
